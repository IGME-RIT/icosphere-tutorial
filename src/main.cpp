...
void checkTime()
{
	...
	double dt = elapsedTime - timebase;
	...
	if (dt > physicsStep)
	{
		...
		{
			fps = frame / (elapsedTime - FPSTime);

			FPSTime = elapsedTime; // Now we set FPSTime = time, so that we have a reference for when we calculated the FPS
		...
		timebase = elapsedTime; // Set timebase = time so we have a reference for when we ran the last physics timestep.
